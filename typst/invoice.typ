...
#let invoice_overall_sum = invoice.jobs.map(job => job.at("price")).sum()
...
Плательщик: #invoice.customer.name ИНН #invoice.customer.INN ОГРН #invoice.customer.OGRN
...
#table(
  ...
  ..for (index, job) in invoice.jobs.enumerate() {(
    table.cell(align: center)[
      ...
      #job.at("task")
    ], table.cell(align: center)[шт], table.cell(align: center)[1], table.cell(align: center)[
